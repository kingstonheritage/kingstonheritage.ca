...
class WidgetkitImage {
	...
	*/
    protected function _options($options) {
		...
		// sanatize options
		$options['format'] = in_array($options['format'], array("gif", "jpg", "jpeg", "png")) ? $options['format']: "auto";
		$options['width']  = is_numeric($options['width']) ? intval($options['width']) : "auto";
		$options['height'] = is_numeric($options['height']) ? intval($options['height']) : "auto";
		...
		} else if ($options['width'] == 'auto' && is_numeric($options['height'])) {
			$options['width'] = @($options['height'] / $source['height']) * $source['width'];
		} else if ($options['height'] == 'auto' && is_numeric($options['width'])) {
